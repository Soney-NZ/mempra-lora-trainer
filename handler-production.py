...
CACHE_DIR = "/workspace/model_cache"  # Persistent cache location
...
def initialize_worker():
    ...
    try:
        ...
        # Step 3: Download and cache Flux model (non-blocking)
        logger.info(f"[INIT] 📥 Downloading Flux model to {CACHE_DIR}...")
        ...
        try:
            ...
            logger.info("[INIT] ✅ Flux model cached successfully")
        except Exception as e:
            logger.warning(f"[INIT] ⚠️ Model download failed: {e} - will retry during job")
            FLUX_MODEL_CACHED = False
