...
def train_lora(job_input: dict):
    try:
        ...
        training_steps = int(job_input.get('training_steps', 600))  # Reduced from 1200 to 600
        learning_rate = float(job_input.get('learning_rate', 0.0002))  # Increased for faster convergence  
        ...
        cmd = [
            ...
            "--train_batch_size=2",  # Increased batch size for faster training
            "--gradient_accumulation_steps=2",  # More efficient gradient updates
            "--mixed_precision=fp16",
            "--save_precision=fp16",
            "--optimizer_type=AdamW8bit",
            "--lr_scheduler=cosine",
            "--lr_warmup_steps=50",  # Reduced warmup for faster start
            "--save_every_n_steps=300",  # Save less frequently 
            "--sample_every_n_steps=150",  # Sample less frequently
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=5400)  # Reduced timeout from 3 hours to 1.5 hours
